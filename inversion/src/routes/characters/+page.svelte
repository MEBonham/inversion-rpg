<script>
    ...
    let revealedCharacters = $derived(
        data.allCharacters.filter((character) => {
            ...
            if (data.allCampaigns.find((campaign) => campaign.id === character.setting)?.creator === profile?.id) return true;
    ...
    <section>
        ...
        {#each displayedCampaigns as campaign}
            <article>
                ...
                <p>
                    {#each revealedCharacters.filter((character) => character.setting === campaign.id) as character, index}
                        <a href={`/characters/${character.slug}`}>{character.character_name}</a>
                        {#if index < revealedCharacters.filter((character) => character.setting === campaign.id).length - 1}
                            <BufferDot />
                        {/if}
                    {/each}
                </p>
